      ...
      `;
        ...
        auto.hooks.afterChangelog.tapPromise(this.name, async ({ commits }) => {
            ...
            await packages.reduce(async (last, { name, path: packagePath }) => {
                ...
                await last;
                if (!fs_1.default.existsSync(path_1.default.join(packagePath, ".all-contributorsrc"))) {
                    auto.logger.verbose.info(`Skipping ${name} because it has no all-contributorsrc file.`);
                    return;
                }
                auto.logger.verbose.info(`Updating contributors for: ${name}`);
                const includedCommits = commits.filter((commit) => commit.files.some((file) => core_1.inFolder(packagePath, file)));
            ...
            if (changedFiles) {
                ...
                auto.logger.verbose.warn('Committed updates to "README.md" and ".all-contributorsrc" files.');
            }
            else {
                auto.logger.verbose.warn('No changes to "README.md" or ".all-contributorsrc" files.');
            }
        });
